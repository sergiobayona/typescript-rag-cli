...
// Unused interfaces should have underscore prefix
interface _Document {
  name: string;
  text: string;
  chunks: string[];
    ...
    this.program
      ...
      .action(async (options: AddCommandOptions & { preserveHtml?: boolean, extractMetadata?: boolean }) => {
        try {
          ...
          // Create a vector index and store the document
          const vectorIndex = new VectorIndex(textEmbeddings);
          this.documentStore.addDocument(name, text, chunks, vectorIndex);
      ...
      .action(async (options: QueryCommandOptions) => {
        try {
          ...
          if (documentToQuery === 'All documents') {
            ...
            // Query specific document
            const document = this.documentStore.getDocument(documentToQuery);
            if (!document) {
              console.log(`Document "${documentToQuery}" not found.`);
              return;
            }
    ...
    this.program
      ...
      .action(async (options: { name?: string }) => {
        try {
          ...
          if (confirmAnswer.confirmRemoval) {
            const removed = this.documentStore.removeDocument(docToRemove);
            if (removed) {
              console.log(`Document "${docToRemove}" has been removed successfully.`);
            } else {
              console.log(`Failed to remove document "${docToRemove}".`);
  ...
  private async mainMenu(): Promise<void> {
    try {
      ...
      while (running) {
        ...
        if (action === 'add') {
          const addCommand = this.program.commands.find(c => c.name() === 'add');
          if (addCommand) {
            await addCommand.parseAsync([]); // Parse with empty args to run the action with no options
          }
        } else if (action === 'list') {
          const listCommand = this.program.commands.find(c => c.name() === 'list');
          if (listCommand) {
            await listCommand.parseAsync([]); // Parse with empty args to run the action with no options
          }
        } else if (action === 'remove') {
          const removeCommand = this.program.commands.find(c => c.name() === 'remove');
          if (removeCommand) {
          ...
          const queryCommand = this.program.commands.find(c => c.name() === 'query');
          if (queryCommand) {
            if (askQuestion) {
